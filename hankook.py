...
import requests
import time
import FinanceDataReader as fdr  # 🌟 새롭게 추가된 금융 데이터 사전 라이브러리
...
# --- [🌟 2. 종목명 <-> 종목코드 번역 기능 (캐싱 적용)] ---
# 웹을 켤 때마다 전체 종목을 다운받으면 느려지므로, @st.cache_data를 써서 메모리에 한 번만 올려둡니다. (CS의 캐싱 개념)
@st.cache_data
def load_stock_dict():
    df = fdr.StockListing('KRX') # 한국 거래소 전체 종목 데이터 불러오기
    return df[['Code', 'Name']]

def name_to_code(input_text, df):
    # 만약 사용자가 '005930' 처럼 숫자를 입력했다면 그대로 통과
    if input_text.isdigit():
        return input_text
    
    # 한글 이름을 입력했다면 데이터프레임에서 검색 후 코드 반환
...
if st.session_state.monitoring:
    ...
    # 🌟 감시 시작 전, 입력한 이름이 진짜 있는 주식인지 사전을 뒤져서 코드(숫자)로 바꿉니다.
